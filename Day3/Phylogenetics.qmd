...
![](images/Mapping_Overview_diag_2.png)
...
\* Reads are in[*blue*]{style="color:blue"}/[*green*]{style="color:green"}. \*

Changes from the reference are highlighted for each read in [*red*]{style="color:red"}.

![](images/bam-ART-pileup.png)
...
![](images/consensus-fasta-Nhighlight.png)
...
The short read assembly is called `CTMA_1441.unicycler-short.fasta`.

Download it from Github using wget from <https://github.com/monicaiabrudan/bacterial-genomics-2025/blob/main/Day3/CTMA_1441.unicycler-short.fasta>
...
Download these two assemblies from Github using wget:

<https://github.com/WCSCourses/GenEpiLAC2025/blob/main/course_data_2025/mapping_and_phylo/CTMA_1441.filtreads.dragonflye.long_4.1_nopolish.contigs.fa>

<https://github.com/WCSCourses/GenEpiLAC2025/blob/main/course_data_2025/mapping_and_phylo/CTMA_1441.filtreads.dragonflye.long_4.1_polish.contigs.fa>
...
\- ask `IQ-TREE` to take account of missing invariant sites using `-fconst $(snp-sites -C clean.full.aln)` - specify an evolutionary model we want `IQ-TREE` to use `-m GTR+F+I`

\- tell `IQ-TREE` to use a maximum of 2 CPUs (threads) and 2GB memory `-T 2 -mem 2G` - perform 1000 ultrafast bootstraps `-B 1000`

\- use sample `M66` as an outgroup `-o M66`

```         
iqtree2 -s clean.full.SNPs.aln -fconst $( snp-sites -C clean.full.aln ) -m GTR+F+I -T 2 -mem 2G -B 1000 -o M66
```
